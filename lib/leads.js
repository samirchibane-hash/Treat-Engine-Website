...
// `event`:
//   checkout_started   — dealer finished /sales/start (or /sales-v2/start) and was sent to Stripe
...
// GHL inbound-webhook trigger for the pre-checkout workflow. This repo is
// public, so anyone can read this URL and post fake contacts to it — set
// LEAD_WEBHOOK_URL in Vercel to move it out of the code (the env var wins).
const DEFAULT_LEAD_WEBHOOK_URL =
  'https://services.leadconnectorhq.com/hooks/A3nxb3oVoXBLP5eXgF6o/webhook-trigger/9e875754-2956-4d34-9d36-bf37bdfa1202';

async function postLead(event, lead) {
  const url = (process.env.LEAD_WEBHOOK_URL || DEFAULT_LEAD_WEBHOOK_URL).trim();
  if (!url) {
...
// The dealer details collected on /sales/start or /sales-v2/start, or null when
// the request came straight from a pricing card (/sales/checkout-v2). Returns
...
function parseLead(raw) {
  ...
  const lead = {
    ...
    // Which funnel: 'sales' (live) or 'sales-v2' (test page).
    source: raw.source === 'sales-v2' ? 'sales-v2' : 'sales',
  };
